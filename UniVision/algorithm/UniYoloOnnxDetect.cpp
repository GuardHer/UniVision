...
bool YoloSegOnnxDetect::ReadModel(const std::string& modelPath, bool isCuda, int cudaID, bool warmUp) {
...
int YoloSegOnnxDetect::PreProcessing(const std::vector<cv::Mat>& srcImgs, std::vector<cv::Mat>& outSrcImgs, std::vector<cv::Vec4d>& params) {
	outSrcImgs.clear();
...
bool YoloSegOnnxDetect::OnnxDetect(const cv::Mat& srcImg, std::vector<OutputParams>& output, cv::Mat& dstImg) {
	...
	if (OnnxBatchDetect(input_data, tenp_output)) {
		...
		dstImg = srcImg.clone();
		DrawPred(dstImg, output);
		return true;
	}
	else return false;
}
bool YoloSegOnnxDetect::OnnxBatchDetect(std::vector<cv::Mat>& srcImgs, std::vector<std::vector<OutputParams>>& output) {
	...
	int socre_array_length = net_width - 4 - (int)_outputMaskTensorShape[1];
	for (int img_index = 0; img_index < srcImgs.size(); ++img_index) {
		...
		for (int r = 0; r < rows; ++r) {    //stride
			...
			if (max_class_socre >= _classThreshold) {
				...
				//rect [x,y,w,h]
				float x = (float)((pdata[0] - params[img_index][2]) / params[img_index][0]);  //x
				float y = (float)((pdata[1] - params[img_index][3]) / params[img_index][1]);  //y
				float w = (float)(pdata[2] / params[img_index][0]);  //w
				float h = (float)(pdata[3] / params[img_index][1]);  //h
...
void YoloSegOnnxDetect::DrawPred(cv::Mat& img, std::vector<OutputParams> result)
{
